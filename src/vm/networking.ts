...
export function cleanupStaleNetworkResources(ownedNamespaces?: Set<string>): void {
  try {
    const nsListOut = execSync("ip netns list 2>/dev/null", { encoding: "utf-8" });
    let staleNamespaces = nsListOut
      .split("\n")
      .map((line) => line.split(" ")[0]?.trim())
      .filter((name): name is string => !!name && /^ns-[A-Za-z0-9_-]+$/.test(name));

    if (ownedNamespaces) {
      staleNamespaces = staleNamespaces.filter((ns) => ownedNamespaces.has(ns));
    }
